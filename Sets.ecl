 ...
 * a huge similarity between the individual C++ functions (three out of four
 * are, in fact, identical except for the data types they use).  Normally
 ...
 * have any visibility to each other nor to a well-defined shared namespace.
        ...
        EXPORT SET OF DATA _MergeData(SET OF DATA the_set1, SET OF DATA the_set2, UNSIGNED2 merge_type) := EMBED(C++ : DISTRIBUTED)
            ...
            class DataInfo
            {
                public:
                    ...
                    DataInfo(int32_t _size, const char* _dataPtr)
                        :   size(_size), dataPtr(_dataPtr)
                        {}

                    DataInfo(const void* ptr)
                        {
                            size = *reinterpret_cast<const int32_t*>(ptr);
                            dataPtr = reinterpret_cast<const char*>(ptr) + sizeof(int32_t);
                        }
            ...
            typedef std::vector<DataInfo> DataList;
            ...
            DataList        sourceObjects1;
            DataList        sourceObjects2;
            unsigned long   totalNumElements = 0;

            while (sourcePtr1 < the_set1 + lenThe_set1)
            {
                DataInfo newElement(sourcePtr1);
            ...
            {
                DataInfo newElement(sourcePtr2);
